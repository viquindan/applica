...
- **Display virtual (Xvfb):** el flujo asistido (`docs/APPLY-ENGINE.md` §4/§5) lanza un navegador HEADFUL - en este VPS sin GUI eso reventaba siempre (`Missing X server or $DISPLAY`) y el worker revertía la aplicación a `pending_review` sin dejar rastro (bug real encontrado en QA, 2026-07-21/22). Servicio systemd `xvfb` (`/etc/systemd/system/xvfb.service`, `Xvfb :99`) + `DISPLAY=:99` en el env de `applica-worker` (`ecosystem.config.js`) - sin esto, el flujo asistido (las 4 ATS objetivo que lo requieren) no puede funcionar en absoluto.
