...
- **APK de Android:** se compila LOCAL con Gradle (no EAS, cuota gratuita agotada) desde una ruta corta fuera de OneDrive (`C:\bld`, ver por qué en `DECISIONS.md`) con `assembleRelease`, y se sube por `scp` directo a `vps-sortcash:/var/www/applica/public/downloads/applica-debug.apk`. La URL de descarga para el usuario es siempre `https://applicaswipe.com/downloads/applica-debug.apk` - no se usa EAS/Expo cloud ni ningún servicio externo para distribuir el APK.
