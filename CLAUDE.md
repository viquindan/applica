...
7. **Antes de tocar CUALQUIER cosa del MOTOR DE BÚSQUEDA (el CORE del producto: extracción de CV, comprensión del perfil, armado del pool, elegibilidad, scoring, umbral, mejora de las bases de ATS), leer `docs/SEARCH-ENGINE.md` COMPLETO.** Ese doc es la fuente de verdad de cómo Applica entiende al usuario y le encuentra las mejores oportunidades. Mapea qué archivo toca qué, lista los invariantes que no se rompen sin decisión de producto (cada uno anclado a un test), y registra el bug real que originó cada regla. Cubre `src/core/{profile,platforms,scoring,pipeline,billing}` y el handler `search_vacancies` de `worker.ts`. La regla 6 (correr `npm test` antes/después + caso nuevo por bug) aplica a TODO este motor, no solo a los 3 archivos que nombra. **Un cambio de UI nunca debe tocar `src/core/`.**
...
- `docs/SEARCH-ENGINE.md` — **el motor de búsqueda a fondo (CORE del producto)**: cómo se entiende al usuario (extracción de CV + perfil) y cómo se usan esos datos para encontrarle las mejores oportunidades (pool, elegibilidad, scoring, umbral, mejora de bases). Invariantes anclados a tests, historial de bugs y sus reglas. Lectura OBLIGATORIA antes de tocar `src/core/{profile,platforms,scoring,pipeline,billing}` o el handler `search_vacancies`.
